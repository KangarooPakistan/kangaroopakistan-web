...
const ContestActions: React.FC<ContestActionsProps> = ({ contest }) => {
  ...
  const handleView = () => {
    console.log("called");
    router.push(`/employee/viewregistered/${contest.id}`);
  };

  const handleAllRegistrationsView = () => {
    console.log("called");
    router.push(`/employee/fetchallregistration/${contest.id}`);
  };
  const handleEdit = () => {
    router.push(
      `/employee/contesttypes/${params.id}/updatecontest/${contest.id}`
    );
  };
