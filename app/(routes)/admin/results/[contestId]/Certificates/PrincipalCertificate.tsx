...
const loadCertificateTemplate = async (): Promise<Uint8Array> => {
  ...
  const response = await fetch("/templates/ikmc/principal_certificates.pdf");
  if (!response.ok) {
...
export async function generatePrincipalCertificate(
  ...
  // const baseTopPosition = schoolLines.length > 1 ? 290 : 305;
  const baseTopPosition = schoolLines.length > 1 ? 280 : 290;
