...
const Register = () => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      console.log("resp");
      console.log(resp);
      const dataBySchoolId = await axios.get(
        `/api/users/allusers/getschoolbyschoolid/${sid}`
      );
      console.log(dataBySchoolId);
      ...
      setYear(contestYear);
      console.log("-KKR-");
      setDistrict(dataBySchoolId.data.district);
      setSchoolName(dataBySchoolId.data.schoolName);
      setSchoolId(dataBySchoolId.data.schoolId);
      setSchoolEmail(dataBySchoolId.data.id);
    };
    fetch();
  }, [id]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (hasDuplicates) {
      ...
      try {
        ...
        router.back();
        toast.success("🦄 Student registered successfully, Please check your email", {
